...
try:
    ...
    from cadquery.importers import importStep, importIges, importBrep
...
class CadKernelAdapter(ABC):
    ...
    # --- Transforms ---
    @abstractmethod
    def translate(self, obj: Any, vector: Tuple[float, float, float]) -> Any:
        pass

    @abstractmethod
    def rotate(self, obj: Any, axis: Tuple[float, float, float], angle: float) -> Any:
        pass

    @abstractmethod
    def scale(self, obj: Any, factor: float) -> Any:
        pass
...
class NoCadAdapter(CadKernelAdapter):
    ...
    translate = _not_implemented
    rotate = _not_implemented
    scale = _not_implemented
...
class CadQueryAdapter(CadKernelAdapter):
    ...
    def translate(self, obj: Any, vector: Tuple[float, float, float]) -> cq.Workplane:
        return cq.Workplane(self._get_solid(obj)).translate(vector)

    def rotate(self, obj: Any, axis: Tuple[float, float, float], angle: float) -> cq.Workplane:
        # CadQuery rotate takes start, end point, and angle
        return cq.Workplane(self._get_solid(obj)).rotate((0,0,0), axis, angle)

    def scale(self, obj: Any, factor: float) -> cq.Workplane:
        return cq.Workplane(self._get_solid(obj)).scale(factor)
    ...
    def tessellate(self, obj: Any, tolerance: float = 0.1, angular_tolerance: float = 0.1) -> Dict[str, Any]:
        ...
        triangulation = solid.tessellate(tolerance=tolerance, angular_tolerance=angular_tolerance)
        vertices = [v.toTuple() for v in triangulation[0]]
        
        # Faces need to be flattened and 0-indexed
        faces = []
        for face_indices in triangulation[1]:
            # Polygons with more than 3 vertices need to be triangulated
            for i in range(len(face_indices) - 2):
                faces.append(face_indices[0])
                faces.append(face_indices[i + 1])
                faces.append(face_indices[i + 2])
        
        # Normals can be calculated from the triangulation
        normals = [n.toTuple() for n in solid.getNormals(triangulation[0])] if triangulation[0] else []
    ...
    def shape_to_brep_string(self, obj: Any) -> Optional[str]:
        ...
        # OCCT's BRepTools::Write to a string stream, then base64 encode
        from OCP.BRepTools import BRepTools
        from OCP.TCollection import TCollection_AsciiString
        from io import BytesIO
        
        # This is a bit of a hack to write to a memory buffer
        # A more direct way might exist but this is reliable
        tmp_file = BytesIO()
        BRepTools.Write_s(solid.wrapped, tmp_file)
        tmp_file.seek(0)
        brep_bytes = tmp_file.read()
        return base64.b64encode(brep_bytes).decode('utf-8')

    def brep_string_to_shape(self, brep_string: str) -> Optional[Any]:
        from OCP.BRepTools import BRepTools
        from OCP.TopoDS import TopoDS_Shape
        from OCP.BRep.BRep_Builder import BRep_Builder
        from io import BytesIO

        brep_bytes = base64.b64decode(brep_string)
        
        # This is also a bit of a hack to read from a memory buffer
        tmp_file = BytesIO(brep_bytes)
        
        shape = TopoDS_Shape()
        builder = BRep_Builder()
        BRepTools.Read_s(shape, tmp_file, builder)
