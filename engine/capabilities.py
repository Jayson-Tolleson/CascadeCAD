...
class Capabilities:
    ...
    def _get_import_caps(self) -> List[str]:
        ...
        if CADQUERY_AVAILABLE:
            formats.extend(["step", "stp", "iges", "igs", "brep"])
    ...
    def _get_operation_caps(self) -> Dict[str, bool]:
        return {
            "translate": False, # PENDING
            "rotate": False, # PENDING
            "scale": False, # PENDING
            "fuse": CADQUERY_AVAILABLE,
            "cut": CADQUERY_AVAILABLE,
            "intersect": CADQUERY_AVAILABLE,
            "fillet": False, # PENDING
            "chamfer": False, # PENDING
            "extrude": False, # PENDING
            "revolve": False, # PENDING
        }