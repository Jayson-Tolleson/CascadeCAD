...
chmod +x "$INSTALL_DIR/scripts/freecad_import_fcstd.py" 2>/dev/null || true
if [[ ! -f "$INSTALL_DIR/scripts/freecad_import_fcstd.py" ]]; then
 echo "ERROR: FCStd import helper is missing" >&2
 exit 1
fi
...
echo "Local test: http://127.0.0.1:8790/CascadeCAD/"
echo "HTTPS target: https://lftr.biz/CascadeCAD/"
echo "Health check: curl http://127.0.0.1:8790/CascadeCAD/healthz"
